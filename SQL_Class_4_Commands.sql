...
-- write a query to get store wise payment yearly
...
-- FROM (
--     SELECT
--         YEAR(payment_date) AS yearly, store_id, SUM(payment_amount) AS total_payment
--     FROM payment
--     GROUP BY YEAR(payment_date), store_id WITH ROLLUP
--     ORDER BY YEAR(payment_date), store_id
-- ) t
-- WHERE store_id IS NULL AND yearly IS NOT NULL;
...
WHERE p.Price < ALL (
    SELECT pr.Price
    FROM Products1 pr
    INNER JOIN Orders1 o ON pr.ProductID = o.ProductID
...
-- Create views in SQL
create view employee_data_for_finance as select emp_id, emp_name,salary from employees1;

select * from employee_data_for_finance;

-- Create logic for department wise salary sum
select dept_name, sum(salary) AS total_salary from employees1 group by dept_name;
create view department_wise_salary as select dept_name, sum(salary) as total_salary from employees1 group by dept_name;
